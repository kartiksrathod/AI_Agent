...
```bash
curl -X POST https://total-checkout.preview.emergentagent.com/api/auth/login \
...
```bash
curl -X POST https://total-checkout.preview.emergentagent.com/api/auth/forgot-password \
  -H "Content-Type: application/json" \
...
### Test Registration:
```bash
curl -X POST https://total-checkout.preview.emergentagent.com/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Test User",
